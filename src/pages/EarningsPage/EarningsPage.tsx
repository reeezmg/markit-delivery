...
const generateOrders = (offsetWeeks = 0) => {
    ...
    baseDate.setDate(today.getDate() - today.getDay() - offsetWeeks * 7);
...
const getWeekRange = (offsetWeeks = 0) => {
    ...
    start.setDate(today.getDate() - today.getDay() - 7 * offsetWeeks);
...
const EarningsPage: React.FC = () => {
    ...
    // -------------------- Order Counts --------------------
    const totalThisWeekOrders = Object.entries(thisWeekOrders as Record<string, Order[]>)
        .slice(0, todayIndex + 1)
        .reduce((sum, [, orders]) => sum + orders.length, 0);

    const totalLastWeekOrders = Object.values(lastWeekOrders as Record<string, Order[]>).reduce(
        (sum, orders: Order[]) => sum + orders.length,
        0
    );
    ...
    return (
        <IonPage className="earnings-page">
            <IonHeader translucent>
                ...
                <IonToolbar className="earnings-header">
                    <IonButtons slot="start">
                        <IonBackButton defaultHref="/HomePage" />
                    </IonButtons>
                    
                    <IonTitle className="earnings-header-title">Earnings</IonTitle>
                </IonToolbar>
            </IonHeader>
                                ...
                                <div className="chart-container">
                                    <ResponsiveContainer width="100%" height={150}>
                                        ...
                                        >
                                            ...
                                            <YAxis />
                                            <Tooltip />
                                            <Bar dataKey="earnings" fill="#a50505" barSize={40} />
                                        </BarChart>
                                    </ResponsiveContainer>
                                </div>
                            </section>
